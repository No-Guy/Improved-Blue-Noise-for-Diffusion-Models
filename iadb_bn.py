...
parser.add_argument("--cov", type=str, default='theirs', help="covariance matrix")
...
if platform.system() == 'Windows':
    ...
    pass
if(opt.cov == 'baseline' or opt.cov == 'untrained'):
    cov_mat_L = np.load(f'covariances/L_cov_{opt.cov}.npz'.format(64, dimension))['x'].astype(np.float32)
else:
    cov_mat_L = np.load(f'covariances/L_cov_{opt.cov}.npy'.format(64, dimension)).astype(np.float32)
print("covariance used: ",opt.cov)
print("model used: ",opt.model if opt.train_or_test == 'test' else 'their',"\n")
...
def main():
    ...
    if train_or_test == 'test':
        ...
        model.load_state_dict(torch.load(f'{output_folder}/{opt.cov}/{opt.model}.ckpt'))
        ...
        if opt.optimize_scheduler_param:
            scheduler_params = np.loadtxt(f'{output_folder}/{opt.cov}/scheduler_params.txt')
        else:
        ...
        for i in tqdm(range(start_batch, num_batch)):
            ...
            if True:
                # plt.figure(1)
                os.makedirs(output_folder + '/{:}/seqs'.format(opt.cov), exist_ok=True)
                for j in range(0, len(sample_all), 1):
                    ...
                    Image.fromarray((sample_plot.permute(1, 2, 0).detach().cpu().numpy()*255).astype(np.uint8)).save(output_folder + '/{:}/seqs/img{:0>5}_step{:}.png'.format(opt.cov, cnt, int((j*100)/1000*opt.nb_steps)))
                ...
                # plt.clf()
                os.makedirs(output_folder + '/{:}/images/{:}'.format(opt.cov,opt.model), exist_ok=True)
                for j in range(cur_batch_size):
                    cnt += 1
                    sample_j = sample[j]
                    sample_j = torch.clamp((sample_j + 1) / 2.0, 0.0, 1.0)
                    Image.fromarray((sample_j.permute(1, 2, 0).detach().cpu().numpy()*255).astype(np.uint8)).save(output_folder + '/{:}/images/{:}/{:0>5}.png'.format(opt.cov,opt.model, cnt))
    ...
    # if(opt.model != 'None'):
    #     model.load_state_dict(torch.load(f'{output_folder}/model_theirs.ckpt'))
    ...
    for current_epoch in tqdm(range(opt.epochs)):
        ...
        # continue
        print('loss: ', np.mean(np.array(losses))/opt.batch_size,'\n')
        ...
        os.makedirs(output_folder + '/{:}'.format(opt.cov),exist_ok=True)
        plt.figure(1)
        plt.plot(np.array(losses)/opt.batch_size)
        # plt.plot(np.mean(np.array(losses)))     # track mean of losses
        plt.savefig(output_folder + '/{:}/losses.png'.format(opt.cov))
        plt.clf()
        np.savetxt(output_folder + '/{:}/losses.txt'.format(opt.cov), np.array(losses))

        plt.figure(1)
        plt.plot(scheduler_params_0)
        plt.plot(scheduler_params_1)
        plt.plot(scheduler_params_2)
        plt.savefig(output_folder + '/{:}/scheduler_params.png'.format(opt.cov))
        plt.clf()

        np.savetxt(f'{output_folder}/{opt.cov}/scheduler_params.txt', scheduler_params.detach().cpu().numpy())
        if(current_epoch % 50 == 0 or current_epoch == 20 or current_epoch == 10 or current_epoch == 999):
            torch.save(model.module.state_dict(), f'{output_folder}/{opt.cov}/{opt.model}_{current_epoch}.ckpt')
